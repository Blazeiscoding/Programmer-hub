...
const Answers = ({
  ...
  const [avatarUrls, setAvatarUrls] = React.useState<{ [key: string]: string }>({});
  React.useEffect(() => {
    const loadAvatars = async () => {
        const urls: { [key: string]: string } = {};  // Initialize with the correct type
        for (let answer of _answers.documents) {
            const initialsBuffer = await avatars.getInitials(answer.author.name, 36, 36);
            const blob = new Blob([initialsBuffer], { type: "image/png" });
            const url = URL.createObjectURL(blob);
            urls[answer.author.$id] = url;
        }
        setAvatarUrls(urls);  // Set the avatar URLs with the correctly typed object
    };
        ...
        <div key={answer.$id} className="flex gap-4">
            ...
            <div className="w-full overflow-auto">
                ...
                <div className="mt-4 flex items-center justify-end gap-1">
                    <picture>
                        <img
                            src={avatarUrls[answer.author.$id] || "/default-avatar.png"}
