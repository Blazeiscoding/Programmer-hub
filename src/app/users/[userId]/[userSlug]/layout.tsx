...
const Layout = async ({
  ...
  const avatarArrayBuffer = await avatars.getInitials(user.name, 200, 200);

  // Convert ArrayBuffer to a Blob
  const blob = new Blob([avatarArrayBuffer], { type: "image/png" });
      ...
      <div className="flex flex-col gap-4 sm:flex-row">
        <div className="w-40 shrink-0">
          <picture className="block w-full">
            <img
              // Use the created URL for the image src
              src={avatarUrl}
              alt={user.name}
